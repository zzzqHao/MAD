...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        if self.base_aug_mag:
            self.base_transform = transforms.RandAugment(num_ops = self.base_aug_mag)
        ...
        if keep_all:
            ...
            self.targets_all = self.targets.copy()
    ...
    def NewClassSelector(self, data, targets, index):
        ...
        index = ind_np.reshape((self.way, self.shot))
        data_chunks = []
        target_chunks = []
        for ind_cl in index:
            data_chunks.append(data[ind_cl])
            target_chunks.append(targets[ind_cl])

        data_tmp = np.vstack(data_chunks)
        targets_tmp = np.hstack(target_chunks)
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            # Applying the strong target transform based on the magnitude
            # if self.strong_target is not None and target in self.strong_target:
            #     strong_transform = self.mag_rand_aug[self.strong_target[target]]
            #     img = strong_transform(img)
            if self.base_aug_mag > 0 and target < 60:
