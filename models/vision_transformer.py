...
__all__ = ['vit_base_patch16_224_21k_ibot', 'vit_base_patch16_224_mae', 'vit_base_patch16_224_mocov3', 
           'vit_base_patch16_224_ibot', 'vit_base_patch16_224_dino',
...
#         B, N, C = x.shape
#         if 'lora' in kwargs:
#             lora_w = kwargs['lora'].cal_delta_w(device=x.device, task_id=kwargs['task_id'], depth_id=kwargs['depth_id'], train=kwargs['train'])
#             w_q = self.qkv.weight.t().chunk(3, dim=1)[0] + lora_w.chunk(3, dim=1)[0]
#             w_k = self.qkv.weight.t().chunk(3, dim=1)[1] + lora_w.chunk(3, dim=1)[1]
#             w_v = self.qkv.weight.t().chunk(3, dim=1)[2] + lora_w.chunk(3, dim=1)[2]
#             w_qkv = torch.cat([w_q, w_k, w_v], dim=1)
#             # w_qkv = self.qkv.weight.t() + lora_w
#             if kwargs['train']:
#                 kwargs['lora'].get_all_w([w_q, w_k, w_v], depth_id=kwargs['depth_id'])
#             qkv = torch.einsum('bnc, cd -> bnd', x, w_qkv) + self.qkv.bias
...
#         if 'lora' in kwargs:
#             lora_w = kwargs['lora'].cal_delta_w(device=x.device, task_id=kwargs['task_id'], depth_id=kwargs['depth_id'], train=kwargs['train'], position='out')
#             w_out = self.proj.weight.t() + lora_w
#             if kwargs['train']:
#                 kwargs['lora'].get_all_w(w_out, depth_id=kwargs['depth_id'], position='out')
#             x = torch.einsum('bnc, cd -> bnd', x, w_out) + self.proj.bias
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, bias=True, drop=0.):
        ...
        # self.fc1_w = torch.zeros((in_features, hidden_features))
        # self.fc2_w = torch.zeros((hidden_features, out_features))
...
class Projection(nn.Module):
    def __init__(self, dim=768, hidden=None, drop=0.0, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()

        if hidden is None:
            self.net = nn.Identity()
        else:
            assert isinstance(hidden, int), 'hidden type should be int'
            self.net = nn.Sequential(
                nn.Linear(dim, dim*hidden),
                act_layer(),
            )
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            mlp_structure=[], projection_hidden=None, attn=Attention):
        """
        Args:
        ...
        # random projection
        if projection_hidden is not None:
            self.projection = Projection(dim=embed_dim, hidden=projection_hidden)
            self.num_features *= projection_hidden
        else:
            self.projection = nn.Identity()

        # Classifier Head
        self.fc_norm = norm_layer(self.num_features) if use_fc_norm else nn.Identity()
        self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.head_copy = {n: [] for n, p in self.head.named_parameters()}
        # self.head_SC = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        if weight_init != 'skip':
    ...
    def forward_features(self, x, task_id=-1, train=False, cls_features=None, lora_id=None):
        ...
        return_forward_layer_tokens = getattr(self, "return_forward_layer_tokens", False)
        forward_layers = list(getattr(self, "forward_layers", []))
        forward_layer_set = set(forward_layers)
        forward_layer_tokens = {}
        
        for i in range(self.depth):
            x = self.blocks[i](x, depth_id=i, train=train)
            if return_forward_layer_tokens and i in forward_layer_set:
                forward_layer_tokens[i] = x

        x = self.norm(x)
        res['x'] = x
        if return_forward_layer_tokens:
            if (self.depth - 1) in forward_layer_set:
                forward_layer_tokens[self.depth - 1] = x
            res['forward_layer_tokens'] = [forward_layer_tokens[i] for i in forward_layers]
    ...
    def get_loss_ortho(self, task_id, device=None, args=None):
        loss = torch.zeros(1).to(device)
        for depth_id in range(self.depth):
            for position in self.lora_layer.position:
                loss = loss + self.lora_layer.loss_ortho(task_id=task_id, device=device, depth=depth_id, position=position, args=args)
        
        return args.ortho * loss
    
    def update_attention(self, task_id, device=None):
        for i in range(self.depth):
            qkv = self.lora_layer.cal_delta_w(device=device, task_id=task_id, depth_id=i, train=False, position='qkv')
            self.blocks[i].attn.qkv.weight.data += qkv.t()
            out = self.lora_layer.cal_delta_w(device=device, task_id=task_id, depth_id=i, train=False, position='out')
            self.blocks[i].attn.proj.weight.data += out.t()
            fc1 = self.lora_layer.cal_delta_w(device=device, task_id=task_id, depth_id=i, train=False, position='fc1')
            self.blocks[i].mlp.fc1.weight.data += fc1.t()
            fc2 = self.lora_layer.cal_delta_w(device=device, task_id=task_id, depth_id=i, train=False, position='fc2')
...
#     #del model.head
#     state_dict = model.state_dict()
#     ckpt = torch.load('../pretrained_ViT/dino_vitbase16_pretrain.pth', map_location='cpu')
#     not_in_k = [k for k in ckpt.keys() if k not in state_dict.keys()]
#     for k in not_in_k:
#         del ckpt[k]
#     state_dict.update(ckpt)
