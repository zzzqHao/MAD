...
def get_command_line_parser():
    ...
    parser.add_argument('-base_pretrain_epochs', type=int, default=0,
                        help='Base-session SupCon warmup epochs before OrCo base alignment')
    parser.add_argument('-incremental_new_loss_weight', type=float, default=0.5,
                        help='Loss weight for current-session new-class samples during incremental training')
    parser.add_argument('-incremental_base_loss_weight', type=float, default=0.0,
                        help='Loss weight for base/old-class samples during incremental training')
    parser.add_argument('-lr_base', type=float, default=0.001)
    parser.add_argument('-lr_pretrain', type=float, default=0.001,
                        help='Learning rate for base_pretrain_epochs. Defaults to lr_base when unset.')
    ...
    parser.add_argument('-min_lr_pretrain', type=float, default=0.0)
    parser.add_argument('-min_lr_new', type=float, default=0.0)
    parser.add_argument('-warmup_epochs_base', type=int, default=3)
    parser.add_argument('-warmup_epochs_pretrain', type=int, default=5)
    parser.add_argument('-warmup_epochs_new', type=int, default=5)
    parser.add_argument('-warmup_start_lr', type=float, default=3e-5)
    parser.add_argument('-optimizer', type=str, default='adamw',
                        choices=['sgd', 'adam', 'adamw'])

    parser.add_argument('-decay', type=float, default=0.0001)
    parser.add_argument('-decay_new', type=float, default=0.0001) 
    ...
    parser.add_argument('-pretrained', type=str2bool, default=True, help='Load pretrained model or not')
    ...
    parser.add_argument('-orco_supcon_temperature', type=float, default=0.07)
...
if __name__ == '__main__':
    ...
    # trainer.save_features_for_tsne()
    # trainer.plot_tsne()
    # trainer.test()
